...
module Mifare
  ...
  attach_function :mifare_classic_read, %i[pointer uchar pointer], :int

  # tag, blocknumber, block data
  attach_function :mifare_classic_write, %i[pointer uchar pointer], :int
  ...
  module Classic
    class Tag < Mifare::Tag
      ...
      def auth(block_num, key_type, key)
        raise Mifare::Error, 'Wrong key type' unless %i[key_a key_b].include? key_type
        raise Mifare::Error, 'Wrong key length' unless [6, 12].include? key.size

        key_ptr = FFI::MemoryPointer.new(:uchar, 6)
        key_ptr.put_bytes(0, key.size == 6 ? key : [key].pack('H*'))

        res = Mifare.mifare_classic_authenticate(@pointer, block_num, key_ptr,
                                                 key_type)
        raise Mifare::Error, "Can't autenticate to block 0x%02x" % block_num if res != 0
      ...
      def read(block_num = nil)
        ...
        raise Mifare::Error, "Can't read block 0x%02x" % block_num if res != 0
      ...
      def write(data, block_num = nil)
        ...
        raise Mifare::Error, "Can't write block 0x%02x" % block_num if res != 0
      end
      ...
      def init_value(value, addr = nil, block_num = nil)
        ...
        raise Mifare::Error, "Can't init value block 0x%02x" % block_num if res != 0
      end

      # returns only value part of value block
      def value(block_num = nil)
        v, = value_with_addr(block_num)
        v
      end
      ...
      def value_with_addr(block_num = nil)
        ...
        raise Mifare::Error, "Can't read value block 0x%02x" % block_num if res != 0
      ...
      def inc(amount = 1, block_num = nil)
        ...
        raise Mifare::Error, "Can't increment block 0x%02x" % block_num if res != 0
      end
      ...
      def dec(amount = 1, block_num = nil)
        ...
        raise Mifare::Error, "Can't decrement block 0x%02x" % block_num if res != 0
      end
      ...
      def transfer(block_num = nil)
        ...
        raise Mifare::Error, "Can't transfer to block 0x%02x" % block_num if res != 0
      end

      # Check's if our tag class is able to handle this LibNFC::Target
      def self.match?(target)
        keys = %i[CLASSIC_1K CLASSIC_1K_EMULATED CLASSIC_1K_ANDROID
