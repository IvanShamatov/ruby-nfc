...
module NFC
  class Reader
    ...
    def connect
      ...
      raise NFC::Error, 'Cant connect to ' << @name if @ptr.null?
    ...
    def discover(*card_types)
      ...
      card_types.inject([]) do |tags, card_type|
        raise NFC::Error, 'Wrong card type' unless card_type.respond_to? :discover
      ...
      loop do
        ...
        0.upto(res - 1) do |i|
          ...
          card_types.each do |card_type|
            next unless card_type.match?(target)
    ...
    def self.all
      ...
      raise NFC::Error, 'No compatible NFC readers found' if len <= 0

      names = ptr.get_bytes(0, 1024 * len).split("\x00").reject(&:empty?)
